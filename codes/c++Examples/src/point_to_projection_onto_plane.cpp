...
#include "cauchy.h"
#include "point_to_projection_onto_plane_tait_bryan_wc_jacobian.h"
#include "point_to_projection_onto_plane_rodrigues_wc_jacobian.h"
#include "point_to_projection_onto_plane_quaternion_wc_jacobian.h"
...
bool initGL(int *argc, char **argv) {
	...
	glutCreateWindow("point_to_projection_onto_plane");
	glutDisplayFunc(display);
	glutKeyboardFunc(keyboard);
	glutMotionFunc(motion);
				...
				for(size_t j = 0 ; j < planes_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < planes_local[i].size(); k++){
							...
							Eigen::Vector3d v_trg_pl(plane_target_global(0,2), plane_target_global(1,2), plane_target_global(2,2));
							Eigen::Vector3d point_on_target_plane(plane_target_global(0,3), plane_target_global(1,3), plane_target_global(2,3));
							...
							point_to_projection_onto_plane_tait_bryan_wc(delta,
									pose.px, pose.py, pose.pz, pose.om, pose.fi, pose.ka,
									point_source_local.x(), point_source_local.y(), point_source_local.z(),
									point_on_target_plane.x(), point_on_target_plane.y(), point_on_target_plane.z(),
									v_trg_pl.x(), v_trg_pl.y(), v_trg_pl.z());

							Eigen::Matrix<double, 3, 6> delta_jacobian;
							point_to_projection_onto_plane_tait_bryan_wc_jacobian(delta_jacobian,
									pose.px, pose.py, pose.pz, pose.om, pose.fi, pose.ka,
									point_source_local.x(), point_source_local.y(), point_source_local.z(),
									point_on_target_plane.x(), point_on_target_plane.y(), point_on_target_plane.z(),
									v_trg_pl.x(), v_trg_pl.y(), v_trg_pl.z());
				...
				for(size_t j = 0 ; j < planes_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < planes_local[i].size(); k++){
							...
							Eigen::Matrix<double, 3, 1> delta;
							point_to_projection_onto_plane_rodrigues_wc(delta,
							...
							Eigen::Matrix<double, 3, 6> delta_jacobian;
							point_to_projection_onto_plane_rodrigues_wc_jacobian(delta_jacobian,
									pose.px, pose.py, pose.pz, pose.sx, pose.sy, pose.sz,
									point_source_local.x(), point_source_local.y(), point_source_local.z(),
									point_on_target_plane.x(), point_on_target_plane.y(), point_on_target_plane.z(),
									v_trg_pl.x(), v_trg_pl.y(), v_trg_pl.z());
				...
				for(size_t j = 0 ; j < planes_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < planes_local[i].size(); k++){
							...
							Eigen::Matrix<double, 3, 1> delta;
							point_to_projection_onto_plane_quaternion_wc(delta,
									pose.px, pose.py, pose.pz, pose.q0, pose.q1, pose.q2, pose.q3,
									point_source_local.x(), point_source_local.y(), point_source_local.z(),
									point_on_target_plane.x(), point_on_target_plane.y(), point_on_target_plane.z(),
									v_trg_pl.x(), v_trg_pl.y(), v_trg_pl.z());

							Eigen::Matrix<double, 3, 7> delta_jacobian;
							point_to_projection_onto_plane_quaternion_wc_jacobian(delta_jacobian,
									pose.px, pose.py, pose.pz, pose.q0, pose.q1, pose.q2, pose.q3,
									point_source_local.x(), point_source_local.y(), point_source_local.z(),
									point_on_target_plane.x(), point_on_target_plane.y(), point_on_target_plane.z(),
									v_trg_pl.x(), v_trg_pl.y(), v_trg_pl.z());
