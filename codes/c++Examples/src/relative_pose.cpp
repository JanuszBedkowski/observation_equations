		...
		case '3':
		{
			...
			for (size_t i = 0; i < odo_edges.size(); i++)
			{
				...
				for (int row = 0; row < 6; row++)
				{
					...
					{
						AtPA.coeffRef(ic_1 + row, ic_1 + col) += AtPAodo(row, col);
						AtPA.coeffRef(ic_1 + row, ic_2 + col) += AtPAodo(row, col + 6);
						AtPA.coeffRef(ic_2 + row, ic_1 + col) += AtPAodo(row + 6, col);
						AtPA.coeffRef(ic_2 + row, ic_2 + col) += AtPAodo(row + 6, col + 6);
					}
				}

				for (int row = 0; row < 6; row++)
				{
					AtPB.coeffRef(ic_1 + row, 0) -= AtPBodo(row, 0);
					AtPB.coeffRef(ic_2 + row, 0) -= AtPBodo(row + 6, 0);
				}
			}
			for (size_t i = 0; i < loop_edges.size(); i++)
			{
				...
				relative_pose_tait_bryan_wc_case1_simplified_1(relative_pose_measurement_loop,
															...
															poses_desired[loop_edges[i].second].fi,
															poses_desired[loop_edges[i].second].ka);
				Eigen::Matrix<double, 12, 12> AtPAloop;
				relative_pose_obs_eq_tait_bryan_wc_case1_AtPA_simplified(AtPAloop,
																		 poses[loop_edges[i].first].px,
																		 poses[loop_edges[i].first].py,
																		 poses[loop_edges[i].first].pz,
																		 poses[loop_edges[i].first].om,
																		 ...
																		 poses[loop_edges[i].second].ka,
																		 1, 1, 1, 1, 1, 1);
				Eigen::Matrix<double, 12, 1> AtPBloop;
				relative_pose_obs_eq_tait_bryan_wc_case1_AtPB_simplified(AtPBloop,
																		 poses[loop_edges[i].first].px,
																		 poses[loop_edges[i].first].py,
																		 poses[loop_edges[i].first].pz,
																		 poses[loop_edges[i].first].om,
				...
				for (int row = 0; row < 6; row++)
				{
					for (int col = 0; col < 6; col++)
					{
						AtPA.coeffRef(ic_1 + row, ic_1 + col) += AtPAloop(row, col);
						AtPA.coeffRef(ic_1 + row, ic_2 + col) += AtPAloop(row, col + 6);
						AtPA.coeffRef(ic_2 + row, ic_1 + col) += AtPAloop(row + 6, col);
						AtPA.coeffRef(ic_2 + row, ic_2 + col) += AtPAloop(row + 6, col + 6);
					}
				}

				for (int row = 0; row < 6; row++)
				{
					AtPB.coeffRef(ic_1 + row, 0) -= AtPBloop(row, 0);
					AtPB.coeffRef(ic_2 + row, 0) -= AtPBloop(row + 6, 0);
				}
			}

			for (int row = 0; row < 6; row++)
			{
				AtPA.coeffRef(row, row) += 10000000000000;
			}
			std::cout << "start solving AtPA=AtPB" << std::endl;
			Eigen::SimplicialCholesky<Eigen::SparseMatrix<double>> solver(AtPA);
