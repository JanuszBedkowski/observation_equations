...
bool initGL(int *argc, char **argv) {
	...
	glutCreateWindow("point_to_projection_onto_line");
	glutDisplayFunc(display);
	glutKeyboardFunc(keyboard);
	glutMotionFunc(motion);
				...
				for(size_t j = 0 ; j < lines_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < lines_local[i].size(); k++){
							...
							Eigen::Vector3d v_trg_ln(line_target_global(0,2), line_target_global(1,2), line_target_global(2,2));
							...
							point_to_projection_onto_line_tait_bryan_wc(delta,
									...
									v_trg_ln.x(), v_trg_ln.y(), v_trg_ln.z());

							Eigen::Matrix<double, 3, 6> delta_jacobian;
							point_to_projection_onto_line_tait_bryan_wc_jacobian(delta_jacobian,
									pose.px, pose.py, pose.pz, pose.om, pose.fi, pose.ka,
									point_source_local.x(), point_source_local.y(), point_source_local.z(),
									point_on_target_line.x(), point_on_target_line.y(), point_on_target_line.z(),
									v_trg_ln.x(), v_trg_ln.y(), v_trg_ln.z());
				...
				for(size_t j = 0 ; j < lines_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < lines_local[i].size(); k++){
							...
							Eigen::Matrix<double, 3, 1> delta;
							point_to_projection_onto_line_rodrigues_wc(delta,
							...
							Eigen::Matrix<double, 3, 6> delta_jacobian;
							point_to_projection_onto_line_rodrigues_wc_jacobian(delta_jacobian,
									pose.px, pose.py, pose.pz, pose.sx, pose.sy, pose.sz,
									point_source_local.x(), point_source_local.y(), point_source_local.z(),
									point_on_target_line.x(), point_on_target_line.y(), point_on_target_line.z(),
									v_trg_ln.x(), v_trg_ln.y(), v_trg_ln.z());
				...
				for(size_t j = 0 ; j < lines_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < lines_local[i].size(); k++){
							...
							Eigen::Matrix<double, 3, 1> delta;
							point_to_projection_onto_line_quaternion_wc(delta,
							...
							point_to_projection_onto_line_quaternion_wc_jacobian(delta_jacobian,
									pose.px, pose.py, pose.pz, pose.q0, pose.q1, pose.q2, pose.q3,
									point_source_local.x(), point_source_local.y(), point_source_local.z(),
									point_on_target_line.x(), point_on_target_line.y(), point_on_target_line.z(),
									v_trg_ln.x(), v_trg_ln.y(), v_trg_ln.z());
