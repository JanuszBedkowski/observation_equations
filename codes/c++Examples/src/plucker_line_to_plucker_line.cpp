...
#include "cauchy.h"
#include "plucker_line_to_plucker_line_tait_bryan_wc_jacobian.h"
#include "plucker_line_to_plucker_line_rodrigues_wc_jacobian.h"
#include "plucker_line_to_plucker_line_quaternion_wc_jacobian.h"
...
bool initGL(int *argc, char **argv) {
	...
	glutCreateWindow("plucker_line_to_plucker_line");
	glutDisplayFunc(display);
	glutKeyboardFunc(keyboard);
	glutMotionFunc(motion);
...
void keyboard(unsigned char key, int /*x*/, int /*y*/) {
	switch (key) {
		...
		case 't':{
			...
			for(size_t i = 0 ; i < lines_local.size(); i++){
				for(size_t j = i+1 ; j < lines_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < lines_local[i].size(); k++){
							Eigen::Affine3d m_line_from = lines_local[i][k];
							Eigen::Vector3d point_on_line_from(m_line_from(0,3), m_line_from(1,3), m_line_from(2,3));
							Eigen::Vector3d s_from(m_line_from(0,0), m_line_from(1,0), m_line_from(2,0));
							Eigen::Vector3d m_from = point_on_line_from.cross(s_from);

							Eigen::Affine3d m_line_to = lines_local[j][k];
							Eigen::Vector3d point_on_line_to(m_line_to(0,3), m_line_to(1,3), m_line_to(2,3));
							Eigen::Vector3d s_to(m_line_to(0,0), m_line_to(1,0), m_line_to(2,0));
							Eigen::Vector3d m_to = point_on_line_to.cross(s_to);

							Eigen::Matrix<double, 6, 1> delta;
							plucker_line_to_plucker_line_tait_bryan_wc(delta,
									...
									m_from.x(), m_from.y(), m_from.z(), s_from.x(), s_from.y(), s_from.z(),
									m_to.x(), m_to.y(), m_to.z(), s_to.x(), s_to.y(), s_to.z());
							Eigen::Matrix<double, 6, 12, Eigen::RowMajor> delta_jacobian;
							plucker_line_to_plucker_line_tait_bryan_wc_jacobian(delta_jacobian,
									pose_from.px, pose_from.py, pose_from.pz, pose_from.om, pose_from.fi, pose_from.ka,
									pose_to.px, pose_to.py, pose_to.pz, pose_to.om, pose_to.fi, pose_to.ka,
									m_from.x(), m_from.y(), m_from.z(), s_from.x(), s_from.y(), s_from.z(),
									m_to.x(), m_to.y(), m_to.z(), s_to.x(), s_to.y(), s_to.z());
				...
				for(size_t j = i+1 ; j < lines_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < lines_local[i].size(); k++){
							...
							Eigen::Matrix<double, 6, 1> delta;
							plucker_line_to_plucker_line_rodrigues_wc(delta,
									...
									m_to.x(), m_to.y(), m_to.z(), s_to.x(), s_to.y(), s_to.z());
							Eigen::Matrix<double, 6, 12, Eigen::RowMajor> delta_jacobian;
							plucker_line_to_plucker_line_rodrigues_wc_jacobian(delta_jacobian,
									pose_from.px, pose_from.py, pose_from.pz, pose_from.sx, pose_from.sy, pose_from.sz,
									pose_to.px, pose_to.py, pose_to.pz, pose_to.sx, pose_to.sy, pose_to.sz,
									m_from.x(), m_from.y(), m_from.z(), s_from.x(), s_from.y(), s_from.z(),
									m_to.x(), m_to.y(), m_to.z(), s_to.x(), s_to.y(), s_to.z());
				...
				for(size_t j = i+1 ; j < lines_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < lines_local[i].size(); k++){
							...
							Eigen::Matrix<double, 6, 1> delta;
							plucker_line_to_plucker_line_quaternion_wc(delta,
									...
									m_from.x(), m_from.y(), m_from.z(), s_from.x(), s_from.y(), s_from.z(),
									m_to.x(), m_to.y(), m_to.z(), s_to.x(), s_to.y(), s_to.z());
							Eigen::Matrix<double, 6, 14, Eigen::RowMajor> delta_jacobian;
							plucker_line_to_plucker_line_quaternion_wc_jacobian(delta_jacobian,
									pose_from.px, pose_from.py, pose_from.pz, pose_from.q0, pose_from.q1, pose_from.q2, pose_from.q3,
									pose_to.px, pose_to.py, pose_to.pz, pose_to.q0, pose_to.q1, pose_to.q2, pose_to.q3,
									m_from.x(), m_from.y(), m_from.z(), s_from.x(), s_from.y(), s_from.z(),
									m_to.x(), m_to.y(), m_to.z(), s_to.x(), s_to.y(), s_to.z());
