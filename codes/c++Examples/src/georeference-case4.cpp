...
#include "metric_camera_colinearity_quaternion_wc_jacobian.h"
...
std::vector<int> georeferenced_ground_control_points;
...
int main(int argc, char *argv[]){
	...
	for(size_t i = 0; i < 6; i++){
		georeferenced_ground_control_points.push_back(rand()%tie_points.size());
	}
...
bool initGL(int *argc, char **argv) {
	...
	glutCreateWindow("metric_camera_colinearity_ba");
	glutDisplayFunc(display);
	glutKeyboardFunc(keyboard);
	glutMotionFunc(motion);
...
void display() {
	...
	for(size_t i = 0; i < georeferenced_ground_control_points.size(); i++){
		glVertex3f(tie_points[georeferenced_ground_control_points[i]].x(), tie_points[georeferenced_ground_control_points[i]].y(), tie_points[georeferenced_ground_control_points[i]].z());
	}
	glEnd();
	glPointSize(1);
...
void keyboard(unsigned char key, int /*x*/, int /*y*/) {
	switch (key) {
		...
		case 'p':{
			...
			for(size_t i = 0; i < georeferenced_ground_control_points.size(); i++){
				tie_points[georeferenced_ground_control_points[i]] = tie_points_tmp[i];
			}
			break;
		}
		case 't':{
			...
			for(size_t i = 0; i < cameras.size(); i++){
				for(size_t j = 0; j < cameras[i].key_points.size(); j++){
					...
					observation_equation_metric_camera_colinearity_tait_bryan_wc(delta, cam_params.ksi_0, cam_params.eta_0, cam_params.c, pose.px, pose.py, pose.pz, pose.om, pose.fi, pose.ka,
							...
							std::floor(cameras[i].key_points[j].ksi), std::floor(cameras[i].key_points[j].eta));
			...
			for(size_t i = 0; i < georeferenced_ground_control_points.size(); i++){
				int ir = tripletListB.size();
				int ic = cameras.size() * 6 + georeferenced_ground_control_points[i] * 3;

				double residual;
				residual_constraint_fixed_optimization_parameter(residual, tie_points[georeferenced_ground_control_points[i]].x(), tie_points[georeferenced_ground_control_points[i]].x());
				Eigen::Matrix<double, 1, 1> jacobian;
				residual_constraint_fixed_optimization_parameter_jacobian(jacobian, tie_points[georeferenced_ground_control_points[i]].x(), tie_points[georeferenced_ground_control_points[i]].x());
				...
				residual_constraint_fixed_optimization_parameter(residual, tie_points[georeferenced_ground_control_points[i]].y(), tie_points[georeferenced_ground_control_points[i]].y());
				residual_constraint_fixed_optimization_parameter_jacobian(jacobian, tie_points[georeferenced_ground_control_points[i]].y(), tie_points[georeferenced_ground_control_points[i]].y());
				...
				residual_constraint_fixed_optimization_parameter(residual, tie_points[georeferenced_ground_control_points[i]].z(), tie_points[georeferenced_ground_control_points[i]].z());
				residual_constraint_fixed_optimization_parameter_jacobian(jacobian, tie_points[georeferenced_ground_control_points[i]].z(), tie_points[georeferenced_ground_control_points[i]].z());
		...
		case 'q':{
			...
			for(size_t i = 0; i < cameras.size(); i++){
				for(size_t j = 0; j < cameras[i].key_points.size(); j++){
					...
					Eigen::Matrix<double, 2, 1> delta;
					observation_equation_metric_camera_colinearity_quaternion_wc(delta, cam_params.ksi_0, cam_params.eta_0, cam_params.c, pose.px, pose.py, pose.pz, pose.q0, pose.q1, pose.q2, pose.q3,
							tie_points[cameras[i].key_points[j].index_to_tie_point].x(), tie_points[cameras[i].key_points[j].index_to_tie_point].y(), tie_points[cameras[i].key_points[j].index_to_tie_point].z(),
							std::floor(cameras[i].key_points[j].ksi), std::floor(cameras[i].key_points[j].eta));

					Eigen::Matrix<double, 2, 10, Eigen::RowMajor> jacobian;
					observation_equation_metric_camera_colinearity_quaternion_wc_jacobian(jacobian, cam_params.ksi_0, cam_params.eta_0, cam_params.c, pose.px, pose.py, pose.pz, pose.q0, pose.q1, pose.q2, pose.q3,
							tie_points[cameras[i].key_points[j].index_to_tie_point].x(), tie_points[cameras[i].key_points[j].index_to_tie_point].y(), tie_points[cameras[i].key_points[j].index_to_tie_point].z(),
							std::floor(cameras[i].key_points[j].ksi), std::floor(cameras[i].key_points[j].eta));
			...
			for(size_t i = 0; i < georeferenced_ground_control_points.size(); i++){
				int ir = tripletListB.size();
				int ic = cameras.size() * 7 + georeferenced_ground_control_points[i] * 3;
