...
bool initGL(int *argc, char **argv) {
	...
	glutCreateWindow("point_to_plane");
	glutDisplayFunc(display);
	glutKeyboardFunc(keyboard);
	glutMotionFunc(motion);
				...
				for(size_t j = 0 ; j < planes_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < planes_local[i].size(); k++){
							...
							Eigen::Vector3d vz(plane_target_global(0,2), plane_target_global(1,2), plane_target_global(2,2));
							Eigen::Vector3d point_on_target_line(plane_target_global(0,3), plane_target_global(1,3), plane_target_global(2,3));
				...
				for(size_t j = 0 ; j < planes_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < planes_local[i].size(); k++){
							...
							Eigen::Matrix<double, 1, 1> delta;
							point_to_plane_rodrigues_wc(delta,
							...
							Eigen::Matrix<double, 1, 6> delta_jacobian;
							point_to_plane_rodrigues_wc_jacobian(delta_jacobian,
									pose.px, pose.py, pose.pz, pose.sx, pose.sy, pose.sz,
									point_source_local.x(), point_source_local.y(), point_source_local.z(),
									point_on_target_line.x(), point_on_target_line.y(), point_on_target_line.z(),
									vz.x(), vz.y(), vz.z());
				...
				for(size_t j = 0 ; j < planes_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < planes_local[i].size(); k++){
							...
							Eigen::Matrix<double, 1, 1> delta;
							point_to_plane_quaternion_wc(delta,
							...
							point_to_plane_quaternion_wc_jacobian(delta_jacobian,
									pose.px, pose.py, pose.pz, pose.q0, pose.q1, pose.q2, pose.q3,
									point_source_local.x(), point_source_local.y(), point_source_local.z(),
									point_on_target_line.x(), point_on_target_line.y(), point_on_target_line.z(),
									vz.x(), vz.y(), vz.z());
