...
#include "cauchy.h"
#include "point_to_point_with_scale_tait_bryan_wc_jacobian.h"
#include "point_to_point_with_scale_rodrigues_wc_jacobian.h"
#include "point_to_point_with_scale_quaternion_wc_jacobian.h"
...
bool initGL(int *argc, char **argv) {
	...
	glutCreateWindow("point_to_point_with_scale");
	glutDisplayFunc(display);
	glutKeyboardFunc(keyboard);
	glutMotionFunc(motion);
...
void display() {
	...
	for(size_t i = 0 ; i < trajectory.size(); i++){
		...
		Eigen::Affine3d m_scale = Eigen::Affine3d::Identity();
		m_scale(0,0) = scales[i];
		m_scale(1,1) = scales[i];
		m_scale(2,2) = scales[i];

		for(size_t j = 0 ; j < points_local[i].size(); j++){
			Eigen::Vector3d vt;
			vt = (m * m_scale) * points_local[i][j];
...
void keyboard(unsigned char key, int /*x*/, int /*y*/) {
	switch (key) {
				...
				for(size_t j = 0 ; j < points_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < points_local[i].size(); k++){
							...
							double delta_y;
							double delta_z;
							point_to_point_with_scale_tait_bryan_wc(delta_x, delta_y, delta_z, pose_1.px, pose_1.py, pose_1.pz, pose_1.om, pose_1.fi, pose_1.ka, pose_2.px, pose_2.py, pose_2.pz, pose_2.om, pose_2.fi, pose_2.ka, p_1.x(), p_1.y(), p_1.z(), p_2.x(), p_2.y(), p_2.z(), s_1, s_2);

							Eigen::Matrix<double, 3, 14, Eigen::RowMajor> jacobian;
							point_to_point_with_scale_tait_bryan_wc_jacobian(jacobian, pose_1.px, pose_1.py, pose_1.pz, pose_1.om, pose_1.fi, pose_1.ka, pose_2.px, pose_2.py, pose_2.pz, pose_2.om, pose_2.fi, pose_2.ka, p_1.x(), p_1.y(), p_1.z(), p_2.x(), p_2.y(), p_2.z(), s_1, s_2);
				...
				for(size_t j = 0 ; j < points_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < points_local[i].size(); k++){
							...
							double delta_y;
							double delta_z;
							point_to_point_with_scale_rodrigues_wc(delta_x, delta_y, delta_z, pose_1.px, pose_1.py, pose_1.pz, pose_1.sx, pose_1.sy, pose_1.sz, pose_2.px, pose_2.py, pose_2.pz, pose_2.sx, pose_2.sy, pose_2.sz, p_1.x(), p_1.y(), p_1.z(), p_2.x(), p_2.y(), p_2.z(), s_1, s_2);

							Eigen::Matrix<double, 3, 14, Eigen::RowMajor> jacobian;
							point_to_point_with_scale_rodrigues_wc_jacobian(jacobian, pose_1.px, pose_1.py, pose_1.pz, pose_1.sx, pose_1.sy, pose_1.sz, pose_2.px, pose_2.py, pose_2.pz, pose_2.sx, pose_2.sy, pose_2.sz, p_1.x(), p_1.y(), p_1.z(), p_2.x(), p_2.y(), p_2.z(), s_1, s_2);
		...
		case 'q':{
				...
				for(size_t j = 0 ; j < points_local.size(); j++){
					if(i != j){
						...
						for(size_t k = 0 ; k < points_local[i].size(); k++){
							...
							double delta_y;
							double delta_z;
							point_to_point_with_scale_quaternion_wc(delta_x, delta_y, delta_z, pose_1.px, pose_1.py, pose_1.pz, pose_1.q0, pose_1.q1, pose_1.q2, pose_1.q3, pose_2.px, pose_2.py, pose_2.pz, pose_2.q0, pose_2.q1, pose_2.q2, pose_2.q3, p_1.x(), p_1.y(), p_1.z(), p_2.x(), p_2.y(), p_2.z(), s_1, s_2);

							Eigen::Matrix<double, 3, 16, Eigen::RowMajor> jacobian;
							point_to_point_with_scale_quaternion_wc_jacobian(jacobian, pose_1.px, pose_1.py, pose_1.pz, pose_1.q0, pose_1.q1, pose_1.q2, pose_1.q3, pose_2.px, pose_2.py, pose_2.pz, pose_2.q0, pose_2.q1, pose_2.q2, pose_2.q3, p_1.x(), p_1.y(), p_1.z(), p_2.x(), p_2.y(), p_2.z(), s_1, s_2);
							...
							tripletListA.emplace_back(ir     , ic_1 + 5, -jacobian(0,5));
							tripletListA.emplace_back(ir     , ic_1 + 6, -jacobian(0,6));
							tripletListA.emplace_back(ir     , ic_1 + 7, -jacobian(0,7));

							tripletListA.emplace_back(ir     , ic_2    , -jacobian(0,8));
							tripletListA.emplace_back(ir     , ic_2 + 1, -jacobian(0,9));
							tripletListA.emplace_back(ir     , ic_2 + 2, -jacobian(0,10));
							tripletListA.emplace_back(ir     , ic_2 + 3, -jacobian(0,11));
							tripletListA.emplace_back(ir     , ic_2 + 4, -jacobian(0,12));
							tripletListA.emplace_back(ir     , ic_2 + 5, -jacobian(0,13));
							tripletListA.emplace_back(ir     , ic_2 + 6, -jacobian(0,14));
							tripletListA.emplace_back(ir     , ic_2 + 7, -jacobian(0,15));
							...
							tripletListA.emplace_back(ir + 1 , ic_1 + 5, -jacobian(1,5));
							tripletListA.emplace_back(ir + 1 , ic_1 + 6, -jacobian(1,6));
							tripletListA.emplace_back(ir + 1 , ic_1 + 7, -jacobian(1,7));

							tripletListA.emplace_back(ir + 1 , ic_2    , -jacobian(1,8));
							tripletListA.emplace_back(ir + 1 , ic_2 + 1, -jacobian(1,9));
							tripletListA.emplace_back(ir + 1 , ic_2 + 2, -jacobian(1,10));
							tripletListA.emplace_back(ir + 1 , ic_2 + 3, -jacobian(1,11));
							tripletListA.emplace_back(ir + 1 , ic_2 + 4, -jacobian(1,12));
							tripletListA.emplace_back(ir + 1 , ic_2 + 5, -jacobian(1,13));
							tripletListA.emplace_back(ir + 1 , ic_2 + 6, -jacobian(1,14));
							tripletListA.emplace_back(ir + 1 , ic_2 + 7, -jacobian(1,15));
							...
							tripletListA.emplace_back(ir + 2 , ic_1 + 5, -jacobian(2,5));
							tripletListA.emplace_back(ir + 2 , ic_1 + 6, -jacobian(2,6));
							tripletListA.emplace_back(ir + 2 , ic_1 + 7, -jacobian(2,7));

							tripletListA.emplace_back(ir + 2 , ic_2    , -jacobian(2,8));
							tripletListA.emplace_back(ir + 2 , ic_2 + 1, -jacobian(2,9));
							tripletListA.emplace_back(ir + 2 , ic_2 + 2, -jacobian(2,10));
							tripletListA.emplace_back(ir + 2 , ic_2 + 3, -jacobian(2,11));
							tripletListA.emplace_back(ir + 2 , ic_2 + 4, -jacobian(2,12));
							tripletListA.emplace_back(ir + 2 , ic_2 + 5, -jacobian(2,13));
							tripletListA.emplace_back(ir + 2 , ic_2 + 6, -jacobian(2,14));
							tripletListA.emplace_back(ir + 2 , ic_2 + 7, -jacobian(2,15));
			...
			tripletListP.emplace_back(ir + 5 , ir + 5, 1000000);
			tripletListP.emplace_back(ir + 6 , ir + 6, 1000000);
			tripletListP.emplace_back(ir + 7 , ir + 7, 1000000);
			...
			Eigen::SparseMatrix<double> matA(tripletListB.size(), trajectory.size() * 8);
			Eigen::SparseMatrix<double> matP(tripletListB.size(), tripletListB.size());
			Eigen::SparseMatrix<double> matB(tripletListB.size(), 1);
			...
			Eigen::SparseMatrix<double> AtPA(trajectory.size() * 8, trajectory.size() * 8);
			Eigen::SparseMatrix<double> AtPB(trajectory.size() * 8, 1);
...
void printHelp() {
	...
	std::cout << "s: add noise to scales" << std::endl;
