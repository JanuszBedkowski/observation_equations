...
RT_wc = matrix44FromTaitBryan(px, py, pz, ka, fi, om)
...
with open("metric_camera_colinearity_euler_ka_fi_om_wc_jacobian.h",'w') as f_cpp:  
    f_cpp.write("inline void metric_camera_colinearity_euler_ka_fi_om_wc(double &ksi, double &eta, double ksi_0, double eta_0, double c, double px, double py, double pz, double om, double fi, double ka, double tie_px, double tie_py, double tie_pz)\n")
    f_cpp.write("{")
    f_cpp.write("ksi = %s;\n"%(ccode(obs[0,0])))
    f_cpp.write("eta = %s;\n"%(ccode(obs[1,0])))
    f_cpp.write("}")
    f_cpp.write("\n")
    f_cpp.write("inline void observation_equation_metric_camera_colinearity_euler_ka_fi_om_wc(Eigen::Matrix<double, 2, 1> &delta, double ksi_0, double eta_0, double c, double px, double py, double pz, double om, double fi, double ka, double tie_px, double tie_py, double tie_pz, double ksi_kp, double eta_kp)\n")
    f_cpp.write("{")
    f_cpp.write("delta.coeffRef(0,0) = %s;\n"%(ccode(obs_eq[0,0])))
    f_cpp.write("delta.coeffRef(1,0) = %s;\n"%(ccode(obs_eq[1,0])))
    f_cpp.write("}")
    f_cpp.write("\n")
    f_cpp.write("inline void observation_equation_metric_camera_colinearity_euler_ka_fi_om_wc_jacobian(Eigen::Matrix<double, 2, 9, Eigen::RowMajor> &j, double ksi_0, double eta_0, double c, double px, double py, double pz, double om, double fi, double ka, double tie_px, double tie_py, double tie_pz, double ksi_kp, double eta_kp)\n")
    f_cpp.write("{")
    for i in range (2):
        for j in range (9):
