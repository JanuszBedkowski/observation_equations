...
transformed_point_1 = (matrix44FromTaitBryan(scale * px_1, scale * py_1, scale * pz_1, om_1, fi_1, ka_1) * point_1)[:-1,:]
transformed_point_2 = (matrix44FromTaitBryan(scale * px_2, scale * py_2, scale * pz_2, om_2, fi_2, ka_2) * point_2)[:-1,:]
#transformed_point_1 = (matrix44FromRodrigues(scale * px_1, scale * py_1, scale * pz_1, sx_1, sy_1, sz_1) * point_1)[:-1,:]
#transformed_point_2 = (matrix44FromRodrigues(scale * px_2, scale * py_2, scale * pz_2, sx_2, sy_2, sz_2) * point_2)[:-1,:]
...
with open("point_to_point_vo_scale_tait_bryan_wc_jacobian.h",'w') as f_cpp:  
    f_cpp.write("inline void point_to_point_vo_scale_tait_bryan_wc(double &delta_x, double &delta_y, double &delta_z, double px_1, double py_1, double pz_1, double om_1, double fi_1, double ka_1, double px_2, double py_2, double pz_2, double om_2, double fi_2, double ka_2, double x_1, double y_1, double z_1, double x_2, double y_2, double z_2, double scale)\n")
    f_cpp.write("{")
    ...
    f_cpp.write("}")
    f_cpp.write("\n")
    f_cpp.write("inline void point_to_point_vo_scale_tait_bryan_wc_jacobian(Eigen::Matrix<double, 3, 1> &j, double px_1, double py_1, double pz_1, double px_2, double py_2, double pz_2)\n")
    f_cpp.write("{")
    for i in range (3):
        for j in range (1):
