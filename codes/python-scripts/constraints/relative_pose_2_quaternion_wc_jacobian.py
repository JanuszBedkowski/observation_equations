...
#RT_wc_m = matrix44FromTaitBryan(px_m, py_m, pz_m, om_m, fi_m, ka_m)
#RT_wc_m = matrix44FromTaitBryan(px_m, py_m, pz_m, sx_m, sy_m, sz_m)
RT_wc_m = matrix44FromQuaternion(px_m, py_m, pz_m, q0_2, q1_2, q2_2, q3_2)
...
R_cw_m=RT_wc_m[:-1,:-1].transpose()
T_wc_m=Matrix([px_m, py_m, pz_m]).vec()
T_cw_m=-R_cw_m*T_wc_m
RT_cw_m=Matrix.hstack(R_cw_m, T_cw_m)
RT_cw_m=Matrix.vstack(RT_cw_m, Matrix([[0,0,0,1]]))

mf = RT_cw_m * RT_cw_1 * RT_wc_2

#parametrization = taitBryanFromMatrix44Case1(mf)
#parametrization = rodriguesFromMatrix44(mf)
parametrization = quaternionFromMatrix44(mf)
t = mf[0 : 3, 3]

#target_value=Matrix([0, 0, 0, 0, 0, 0])
#target_value=Matrix([0, 0, 0, 0, 0, 0])
target_value=Matrix([0, 0, 0, 0, 0, 0, 0])
model_function=t.col_join(parametrization)
delta=target_value-model_function
delta_jacobian=delta.jacobian(all_symbols)

with open("relative_pose_2_quaternion_wc_jacobian.h",'w') as f_cpp:  
    f_cpp.write("inline void relative_pose_2_obs_eq_quaternion_wc(Eigen::Matrix<double, 7, 1> &delta, double px_1, double py_1, double pz_1, double q0_1, double q1_1, double q2_1, double q3_1, double px_2, double py_2, double pz_2, double q0_2, double q1_2, double q2_2, double q3_2, double px_m, double py_m, double pz_m, double q0_m, double q1_m, double q2_m, double q3_m)\n")
    f_cpp.write("{")
    for i in range (7):
        f_cpp.write("delta.coeffRef(%d,%d) = %s;\n"%(i, 0, ccode(delta[i])))
    f_cpp.write("}")
    f_cpp.write("\n")
    f_cpp.write("inline void relative_pose_2_obs_eq_quaternion_wc_jacobian(Eigen::Matrix<double, 7, 14, Eigen::RowMajor> &j, double px_1, double py_1, double pz_1, double q0_1, double q1_1, double q2_1, double q3_1, double px_2, double py_2, double pz_2, double q0_2, double q1_2, double q2_2, double q3_2, double px_m, double py_m, double pz_m, double q0_m, double q1_m, double q2_m, double q3_m)\n")
    f_cpp.write("{")
    for i in range (7):
        for j in range (14):
