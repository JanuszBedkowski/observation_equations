...
with open("distance_point_to_plane_rodrigues_wc_jacobian.h",'w') as f_cpp:  
    f_cpp.write("inline void delta_distance_point_to_plane_rodrigues_wc(Eigen::Matrix<double, 1, 1> &delta, double px, double py, double pz, double sx, double sy, double sz, double x, double y, double z, double a, double b, double c, double d)\n")
    f_cpp.write("{")
    f_cpp.write("delta.coeffRef(0,0) = %s;\n"%(ccode(delta[0,0])))
    f_cpp.write("}")
    f_cpp.write("\n")
    f_cpp.write("inline void delta_distance_point_to_plane_rodrigues_wc_jacobian(Eigen::Matrix<double, 1, 6> &j, double px, double py, double pz, double sx, double sy, double sz, double x, double y, double z, double a, double b, double c, double d)\n")
    f_cpp.write("{")
    for i in range (1):
        for j in range (6):
