inline void point_to_line_tait_bryan_wc(Eigen::Matrix<double, 2, 1> &delta, double px, double py, double pz, double om, double fi, double ka, double xsl, double ysl, double zsl, double xtg, double ytg, double ztg, double vxx, double vxy, double vxz, double vyx, double vyy, double vyz)
{delta.coeffRef(0,0) = -vxx*(px + xsl*cos(fi)*cos(ka) - xtg - ysl*sin(ka)*cos(fi) + zsl*sin(fi)) - vxy*(py + xsl*(sin(fi)*sin(om)*cos(ka) + sin(ka)*cos(om)) + ysl*(-sin(fi)*sin(ka)*sin(om) + cos(ka)*cos(om)) - ytg - zsl*sin(om)*cos(fi)) - vxz*(pz + xsl*(-sin(fi)*cos(ka)*cos(om) + sin(ka)*sin(om)) + ysl*(sin(fi)*sin(ka)*cos(om) + sin(om)*cos(ka)) + zsl*cos(fi)*cos(om) - ztg);
delta.coeffRef(1,0) = -vyx*(px + xsl*cos(fi)*cos(ka) - xtg - ysl*sin(ka)*cos(fi) + zsl*sin(fi)) - vyy*(py + xsl*(sin(fi)*sin(om)*cos(ka) + sin(ka)*cos(om)) + ysl*(-sin(fi)*sin(ka)*sin(om) + cos(ka)*cos(om)) - ytg - zsl*sin(om)*cos(fi)) - vyz*(pz + xsl*(-sin(fi)*cos(ka)*cos(om) + sin(ka)*sin(om)) + ysl*(sin(fi)*sin(ka)*cos(om) + sin(om)*cos(ka)) + zsl*cos(fi)*cos(om) - ztg);
}
inline void point_to_line_tait_bryan_wc_jacobian(Eigen::Matrix<double, 2, 6> &j, double px, double py, double pz, double om, double fi, double ka, double xsl, double ysl, double zsl, double xtg, double ytg, double ztg, double vxx, double vxy, double vxz, double vyx, double vyy, double vyz)
{j.coeffRef(0,0) = -vxx;
j.coeffRef(0,1) = -vxy;
j.coeffRef(0,2) = -vxz;
